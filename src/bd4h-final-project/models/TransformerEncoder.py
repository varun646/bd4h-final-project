...
class Embeddings(nn.Module):
    def __init__(self, hidden_size, vocab_size, max_position_size, max_segment, word_emb_padding_idx, dropout_rate=0.1):
...
class SingleStageTransformerEncoder(nn.Module):
    def __init__(self, n_layer, hidden_size, num_attention_heads, vocab_size, max_position_size, 
                 max_segment, word_emb_padding_idx, dropout=0.1):
        super(SingleStageTransformerEncoder, self).__init__()
        self.embeddings = Embeddings(hidden_size, vocab_size, max_position_size, max_segment, word_emb_padding_idx)
        self.multihead_attention_encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_size, nhead=num_attention_heads, dropout=dropout)
        layer_norm = nn.LayerNorm(hidden_size)
        self.transformer_encoder = nn.TransformerEncoder(self.multihead_attention_encoder_layer, num_layers=n_layer, norm=layer_norm)
        
    def forward(self, x):
        emb, padding_mask = self.embeddings(x)
        emb = self.transformer_encoder(emb.transpose(0,1), src_key_padding_mask=padding_mask).transpose(0,1)
        return emb  # [1, seq_len, hidden_size]


class MultiStageTransformerEncoder(nn.Module):
    def __init__(self, n_layer1, n_layer2, hidden_size, num_attention_heads, vocab_size, max_position_size1, max_position_size2, max_segment, word_emb_padding_idx, dropout, device=torch.device('cpu:0')):
        ...
        self.encoder1 = SingleStageTransformerEncoder(n_layer1, hidden_size, num_attention_heads, vocab_size, max_position_size1, max_segment, word_emb_padding_idx, dropout)
    ...
    def forward(self, input_med_ids_list):
        ...
        emb = torch.zeros(1, self.max_position_size2, self.hidden_size, device=self.device)
        ...
        for i, input_ids_list in enumerate(input_med_ids_list):
            ...
            emb[0, i] = processed_emb[:, 0].squeeze(0)  # Squeeze to remove batch dimension
...
class TransformerEncoder(nn.Module):
    def __init__(self,  n_layer, hidden_size, num_attention_heads, vocab_size, max_position_size, max_segment, word_emb_padding_idx, dropout=0.1):
        super(TransformerEncoder, self).__init__()
        self.embeddings = Embeddings(hidden_size, vocab_size, max_position_size, max_segment, word_emb_padding_idx)
        self.layer_norm = nn.LayerNorm(hidden_size)
        self.multihead_attention_encoder = nn.TransformerEncoderLayer(d_model=hidden_size, nhead=num_attention_heads, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(self.multihead_attention_encoder, num_layers=n_layer)
    ...
    def forward(self, x):
        ...
        transformer_encoding = self.transformer_encoder(embeddings.transpose(0, 1), src_key_padding_mask=padding_mask).transpose(0, 1)
