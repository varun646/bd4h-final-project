...
from bd4hFinalProject.models.layers import (
    TransformerEncoder,
    MultiStageTransformerEncoder,
    MulAttention,
)
...
class DrugRec(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        trans_dim = emb_dim
        self.transformer_dp = TransformerEncoder(
            n_layer=2,
            hidden_size=trans_dim,
            num_attention_heads=4,
            vocab_size=28996,
            max_position_size=576,
            max_segment=40,
            word_emb_padding_idx=0,
            dropout=0.1,
        )
        self.transformer_s = TransformerEncoder(
            n_layer=2,
            hidden_size=trans_dim,
            num_attention_heads=4,
            vocab_size=28996,
            max_position_size=256,
            max_segment=40,
            word_emb_padding_idx=0,
            dropout=0.1,
        )
        ...
        self.diag_seq_enc = MulAttention(hidden_size=emb_dim, device=device)
        self.pro_seq_enc = MulAttention(hidden_size=emb_dim, device=device)
        self.sym_seq_enc = MulAttention(hidden_size=emb_dim, device=device)
        ...
        if args.fix_smi_rep:
            self.med_rep = input_smiles_init_rep.to(device)
        else:
            self.med_rep = torch.tensor(input_smiles_init_rep, requires_grad=True).to(
                device
            )
    ...
    def forward(self, input, type):
        ...
        patient_representations = torch.cat(
            [diag_out, pro_out, sym_out], dim=-1
        ).squeeze(dim=0)  # (seq, dim*3)
        ...
        if self.args.CI and type == "train":
            ...
            patient_representations0 = torch.cat(
                [diag_out, pro_out, torch.zeros_like(sym_out)], dim=-1
            ).squeeze(dim=0)  # (seq, dim*3)
            ...
            if not self.args.multivisit:
                ...
                sym_representations_ = torch.zeros((1, self.args.dim)).to(
                    self.device
                )

                for n, i in enumerate(idx):
                    sym_ids = [self.sym_input_ids[self.sym_list[i]].to(self.device)]
                    sym_representations_ += (
                        s_prob[n] * self.transformer_s(sym_ids)[:, 0, :]
                    )  # (1,dim)
                sym_representations_ = sym_representations_.repeat(
                    len(input), 1
                )  # (seq, dim)

            else:
                sym_representations_ = torch.zeros(
                    (len(input), self.args.dim)
                ).to(self.device)
                for j in range(len(input)):
                    if j == 0:
                        s_count = np.random.multinomial(
                            self.args.k_ci, self.sym_prob
                        )
                        idx = np.nonzero(s_count)[0]
                        s_prob = s_count[idx] / self.args.k_ci
                        for n, i in enumerate(idx):
                            sym_ids = [
                                self.sym_input_ids[self.sym_list[i]].to(self.device)
                            ]
                            sym_representations_[[j]] += (
                                s_prob[n] * self.transformer_s(sym_ids)[:, 0, :]
                            )  # (1,dim)
                    else:
                        s_count_ = np.zeros_like(s_count)
                        for j_ in range(len(s_count[idx])):
                            s_count_ += np.random.multinomial(
                                s_count[idx][j_], self.sym_co_prob[idx[j_]]
                            )
                        ...
                        for n_, i_ in enumerate(idx_):
                            sym_ids_ = [
                                self.sym_input_ids[self.sym_list[i_]].to(
                                    self.device
                                )
                            ]
                            sym_representations_[[j]] += (
                                s_prob_[n_] * self.transformer_s(sym_ids_)[:, 0, :]
                            )  # (1,dim)
                        s_count, idx = s_count_, idx_

            # do(s_t) vs do(s_0)
            new_query = self.query_sym(
                torch.cat([query, sym_representations_], dim=-1)
            )
            new_query0 = self.query_sym(
                torch.cat([query0, sym_representations_], dim=-1)
            )
        ...
        neg_pred_prob = torch.einsum(
            "nc,nk->nck", [neg_pred_prob, neg_pred_prob]
        )  # [seq, voc_size, voc_size]

        batch_neg = (
            1
            / self.tensor_ddi_adj.shape[0]
            * neg_pred_prob.mul(self.tensor_ddi_adj).sum(dim=[1, 2]).mean()
        )  # (seq,).mean()
