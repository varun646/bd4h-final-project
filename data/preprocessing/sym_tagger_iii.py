        ...
        for subtree in result.subtrees():
            if subtree.label() == "NP":
                ...
                t_sent = t_sent.replace(
                    noun_phrases_list, "<c>" + noun_phrases_list + "</c>", 1
                )
        return t_sent
...
if __name__ == "__main__":
    notes = pd.read_csv(
        "NOTEEVENTS.csv", usecols=["SUBJECT_ID", "HADM_ID", "CATEGORY", "TEXT"]
    )
    adm = pd.read_csv("ADMISSIONS.csv", usecols=["SUBJECT_ID", "HADM_ID", "DIAGNOSIS"])
    data = dill.load(open("./output/data_iii_sym0.pkl", "rb"))

    notes1 = notes[notes.CATEGORY == "Discharge summary"].sort_values(
        by=["SUBJECT_ID", "HADM_ID"]
    )
    notes2 = (
        notes1.groupby(by=["SUBJECT_ID", "HADM_ID"])["TEXT"]
        .apply("\n".join)
        .reset_index()
    )
    ...
    data2["SYM_LIST"] = data2["TEXT"].progress_apply(text_to_symptom) + data2[
        "DIAGNOSIS"
    ].map(text_to_symptom)
    ...
    data = data3.merge(
        data_lg2[["SUBJECT_ID"]], on="SUBJECT_ID", how="inner"
