...
def combine_process(med_pd, diag_pd, pro_pd):
    ...
    diag_pd = diag_pd0.merge(
        diag_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["ICD9_TEXT"]
        .unique()
        .reset_index(),
    ...
    med_pd0 = (
        med_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["ATC3"].unique().reset_index()
    )
    med_pd = med_pd0.merge(
        med_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["SMILES"].unique().reset_index(),
        on=["SUBJECT_ID", "HADM_ID"],
        how="inner",
    )
    pro_pd0 = (
        pro_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["PRO_CODE"].unique().reset_index()
    )
    pro_pd = pro_pd0.merge(
        pro_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["PRO_TEXT"].unique().reset_index(),
        on=["SUBJECT_ID", "HADM_ID"],
        how="inner",
    )
    ...
    print(
        data.columns
    )  # ['SUBJECT_ID', 'HADM_ID', 'ICD9_CODE', 'ICD9_TEXT', 'ATC3', 'SMILES', 'PRO_CODE', 'PRO_TEXT', 'ATC3_num']
